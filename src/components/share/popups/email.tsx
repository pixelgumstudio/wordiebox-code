...
export default function EmailPopup({
  ...
  // const shareEmail = () => {
  //   setEmailLink(`mailto:?subject=Shared%20Text&body=${encodeURIComponent(password)}`)
  //   submitted(false);
  // };
  ...
  const handleSend = () => {
    ...
    if (email?.match(emailRegex)) {
      email !== "" &&
        !error &&
        axios
          .post("https://wordie-box-backend.onrender.com/api/v1/password-generation", payload)
          .then((response) => {
  ...
  return (
    visible && (
      <div className="w-[100vw] h-[100vh] flex items-center justify-center bg-overlay fixed top-0 left-0 z-20">
        <div className="relative p-8 bg-[#F8F7F1] border text-center border-[#1C1C1C] w-full max-w-fit">
          <Image
            src={Close}
            onClick={closeForm}
            alt=""
